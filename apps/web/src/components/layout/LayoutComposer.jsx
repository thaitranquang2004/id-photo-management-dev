...
function SortableTile({ id, url, widthMm, heightMm }) {
...
// Kéo-sắp-xếp ảnh đã duyệt lên khung A4 đúng tỉ lệ rồi in trực tiếp từ trình duyệt.
export default function LayoutComposer({ photos, widthMm, heightMm }) {
  ...
  function buildTiles() {
    const next = [];
    usablePhotos.forEach((p) => {
      for (let i = 0; i < copies; i += 1) {
        next.push({ key: `${p.id}-${i}-${Math.random().toString(36).slice(2, 7)}`, url: p.url });
      }
    });
    setTiles(next);
  }
  ...
  useEffect(() => {
    if (tiles.length === 0 && usablePhotos.length > 0) buildTiles();
    ...
    <div className="layout-composer">
      <Row className="g-3 align-items-end no-print">
        ...
        <Col xs={12} md={3} className="d-flex gap-2 flex-wrap">
          <Button variant="outline-secondary" onClick={buildTiles}>Dàn lại</Button>
          <Button onClick={() => window.print()}>
            <Printer size={16} aria-hidden="true" /> In A4
          </Button>
          <Button variant="outline-primary" onClick={downloadA4} disabled={downloading}>
            <Download size={16} aria-hidden="true" /> {downloading ? 'Đang tải...' : 'Tải'}
          </Button>
        </Col>
      </Row>
      <p className="text-muted small no-print mt-2">
        Ảnh {w}×{h}mm · ~{capacity} ô/trang · {tiles.length} ô đang xếp. Kéo để sắp xếp lại, rồi bấm <strong>In A4</strong>
        {' '}(trong hộp thoại in chọn khổ <strong>A4</strong>, lề <strong>None/Không</strong> để đúng kích thước).
      </p>
        ...
        <div className="a4-print-area" style={{ padding: `${marginMm}mm`, gap: `${gapMm}mm` }}>
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={tiles.map((t) => t.key)} strategy={rectSortingStrategy}>
              {tiles.map((t) => (
                <SortableTile key={t.key} id={t.key} url={t.url} widthMm={w} heightMm={h} />
              ))}
            </SortableContext>
          </DndContext>
        </div>
